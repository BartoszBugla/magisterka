...
ARCHITECTURE_VERSION = "dual_head_v1"
SENTIMENT_IGNORE_INDEX = -100


class FocalLoss(nn.Module):
    """Focal cross-entropy with optional per-class weights and ignore_index."""

    def __init__(self, alpha: torch.Tensor | None = None, gamma: float = 2.0, ignore_index: int = -100):
        super().__init__()
...
class ABSAModel(nn.Module):
    """Dual-head per-aspect ABSA classifier.

    Shared encoder -> per-aspect mention head (binary) + sentiment head (3-class).
    Sentiment loss is masked to mentioned positions only.
    """
    ...
    def __init__(
        ...
        self.mention_heads = nn.ModuleList([nn.Linear(h, num_mention_classes) for _ in range(num_aspects)])
        self.sentiment_heads = nn.ModuleList([nn.Linear(h, num_sentiment_classes) for _ in range(num_aspects)])

        self.mention_loss_fn = nn.CrossEntropyLoss(weight=mention_pos_weight)
        self.sentiment_loss_fn = FocalLoss(alpha=sentiment_class_weights, gamma=focal_gamma, ignore_index=SENTIMENT_IGNORE_INDEX)
    ...
    def forward(
        ...
        if mention_labels is None and sentiment_labels is None:
            return {"mention_logits": mention_logits, "sentiment_logits": sentiment_logits}

        loss = x.sum() * 0.0
        if mention_labels is not None:
            loss = loss + self.mention_loss_weight * self.mention_loss_fn(
                mention_logits.view(-1, self.num_mention_classes), mention_labels.view(-1))
        if sentiment_labels is not None:
            loss = loss + self.sentiment_loss_weight * self.sentiment_loss_fn(
                sentiment_logits.view(-1, self.num_sentiment_classes), sentiment_labels.view(-1))

        return {"loss": loss, "mention_logits": mention_logits, "sentiment_logits": sentiment_logits}
